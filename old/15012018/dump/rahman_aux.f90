module auxiliary
    ...
    subroutine leapfrog(x_atomd, v_atomd, forced, kineticd, temp)
        use parameters
        implicit none
        integer(8)                      :: i, j, k
...
!                 x_atomd(i,j) = 2.0_8*x_atomd(i,j) - x_past + forced(i,j)*t_step**2
...
!         end do
    ...
    subroutine radial_dist(numd, x_atomsd, r_histd)
        ...
        if (numd==1) then
            open(unit=99, file='rdf_argon.dat', status='replace', action='write')
